...
export default function CreatePost({ userId, user }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!post?.id) throw new Error('สร้างโพสต์ไม่สำเร็จ: ไม่พบ post id')
      ...
      // ดึงโพสต์จริงกลับมาอีกครั้ง เพื่อให้มีรูป + likes + comments ครบ
      ...
      window.dispatchEvent(
        new CustomEvent('new-post', { detail: fullPost })
      )
      ...
      alert(error?.message || JSON.stringify(error) || 'สร้างโพสต์ไม่สำเร็จ')
    } finally {
      setLoading(false)
    }
  }
